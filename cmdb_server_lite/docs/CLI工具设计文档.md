...
1.1 **表头定位（规则 4 补强，§5.6.3 表头鲁棒性）**：扫描前 5 行，定位首个「首单元格为合法英文标识符（`validate_identifier` 通过）」的行作为表头；其前的说明行（如中文标题行）自动跳过，其后的重复表头行（与表头完全相同的行）亦作为非数据跳过，避免把表头/说明行误当作实例数据、生成 `bk_inst_name='bk_inst_name'` 之类的脏实例。前 5 行均未找到英文表头 → 问题记录 `文件无有效表头（前 5 行未找到英文表头行）`。
