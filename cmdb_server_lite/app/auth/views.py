...
def config():
    """前端鉴权配置：告知前端可用的 token 承载方式，前端据此决定
Authorization: Bearer / X-Lite-Token / query 的携带策略。"""
    cfg = get_config()
    return _ok({
        'skip_login': cfg.SKIP_LOGIN,
        'auth_bearer': cfg.AUTH_BEARER,
        'auth_token_query': cfg.AUTH_TOKEN_QUERY,
        'payload_order': cfg.AUTH_PAYLOAD_ORDER,
        'token_max_age': cfg.TOKEN_MAX_AGE,
...
@auth_bp.route('/renew', methods=['GET', 'POST'])
def renew():
    """续期（keep-alive）：当前 token 仍有效时重签一份新令牌并返回。

    - itsdangerous 的时间戳在 make_token 时写入 token 内部，load_token 仅按
      max_age 校验「now − 签发时刻」。因此只要「在有效期内」重新签发，令牌的
      有效期就会相对当前时刻滚动（sliding session）。
    - 仅当用户仍持有「有效」token 才重签；已过期则 current_user_payload() 返回
      None → 返回 1302100，由前端统一跳登录。
    - 前端按「用户活跃」触发（mousemove/keydown 等），实现「使用时保活、闲置不续」。
    """
    cfg = get_config()
    if cfg.SKIP_LOGIN:
        return _ok({
