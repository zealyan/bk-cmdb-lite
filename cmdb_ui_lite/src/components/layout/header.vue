...
import { clearToken, getToken, getSkipLogin, getUserName, setUserName } from '@/auth'
  ...
  methods: {
    ...
    getHeaderLink(nav) {
      ...
      if (nav.id === MENU_BUSINESS) {
        ...
        const bizId = (storeBizId && String(storeBizId) !== '0') ? String(storeBizId) : getCachedBizId(this.userName)
    ...
    async loadUser() {
      ...
      if (cached) {
        this.$store.commit('user/setUser', { name: cached })
        return
      }
      // 未登录（无 token 且非免登录模式）或处于登录页：不回源 /me，
      // 避免「无 token 的 /me 返回 1302100」被响应拦截器误判为「会话失效」而错误跳转。
      // 用户名仅在登录成功后由 login 页写入 localStorage，此处无需主动探测；
      // 免登录模式（skipLogin）下仍需 /me 取 admin 用户名，故放行。
      if (this.$route.path === '/login') return
      if (!getToken() && !getSkipLogin()) return
      try {
        const info = await me()
