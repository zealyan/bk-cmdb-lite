<!--
 * 转移到业务模块弹框的可拖拽分隔布局组件（lite 移植版）
 * 原项目: src/ui/src/components/ui/other/resize.vue (cmdb-resize-layout)
 *
 * lite 适配说明：
 * - 原项目该组件为全局注册（name: cmdb-resize-layout），lite 在精简时误删，
 *   导致 module-selector / host-selector-topology / across-business-module-selector
 *   三处 <cmdb-resize-layout> 引用悬空、且左侧树与右侧面板的 #dcdee5 分隔线丢失。
 * - 此处按原实现 1:1 复刻，并在 main.js 全局注册，恢复拖拽 resize 与分隔线。
-->
