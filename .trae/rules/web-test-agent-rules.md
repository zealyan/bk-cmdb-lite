...
- **每次任务必须先由人工确认是否进行 e2e 测试**：智能体在收到任何 Web 测试 / 验证类任务时，**不得自行默认执行 e2e（Playwright 浏览器自动化）测试**，须先暂停并向用户确认「本次是否需要执行 e2e 测试」。
- 仅当用户**明确许可或主动要求**时，才进入 e2e 流程（见下文执行路径）。
- 当用户选择「不执行 e2e」时，改为**代码级核对**（构建产物校验、源码与原项目比对、静态检查等），并直接汇报核对结论，不启动浏览器自动化。
...
```
0. 确认 e2e 测试意向（人工确认，见上方「前提」）
   - 先向用户确认：本次是否需要执行 e2e（Playwright 浏览器自动化）测试
   - 用户选择「不执行 e2e」→ 改为代码级核对（构建产物、源码比对、静态检查），直接汇报结论，不进入 e2e 流程
   - 用户明确许可 / 主动要求 → 继续以下步骤
