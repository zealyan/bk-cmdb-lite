...
|------|------|
| cc_ObjDes | 模型定义表 |
| cc_ObjAttDes | 模型属性定义表 |
| cc_AsstDes | 关联类型定义表 |
| cc_ObjAsst | 对象关联关系表 |
| cc_InstAsst_0_pub | 实例关联关系表 |
| cc_ObjectBase_0_pub_{obj_id} | 各模型实例表 |
...
| 文件路径 | 说明 |
|---------|------|
| `$BK_CMDB/src/common/tablenames.go` | 数据库表名常量 |
| `$BK_CMDB/src/ui/src/dictionary/model-constants.js` | 模型常量 |
| `$BK_CMDB/src/ui/src/dictionary/property-constants.js` | 属性常量 |
| `$BK_CMDB/docs/db/` | 数据库设计文档 |
...
1. **分析源数据结构** (前端 Mock JSON `$CMDB_UI_LITE/src/assets/json/`)
2. **创建目标表结构** (DuckDB)
3. **编写迁移脚本** (`$CMDB_SERVER_LITE/migrate_*.py`)
4. **执行迁移并验证**
...
# 验证数据库
python3 -c "import duckdb; conn = duckdb.connect('cmdb.duckdb'); print(conn.execute('SHOW TABLES').fetchall())"
```
